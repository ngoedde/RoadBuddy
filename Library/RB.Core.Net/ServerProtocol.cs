...
{
    public ServerProtocol(IMessageAllocator allocator, IMessagePoster poster) : base(allocator, poster,
        ServerMessageDecoder.Shared, ServerMessageEncoder.Shared)
    {
    }
    ...
    public override bool Initialize(int receiverId)
    {
        ...
        if ((_context.Options & ProtocolOptions.KeyExchange) != 0)
        {
            ...
            _context.LocalPublic =
                KeyExchangeHelper.G_pow_X_mod_P(_context.Generator, _context.Private, _context.Prime);
    ...
    protected override KeyExchangeResult OnKeyExchangeReq(Message msg)
    {
        ...
        if (!PostKeyChallenge(msg.SenderID, localSignature))
            return KeyExchangeResult.InvalidState;
