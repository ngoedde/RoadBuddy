...
public record PackBlock
{
    ...
    public byte[] ToByteArray()
    {
        ...
        for (var i = 0; i < Entries.Length; i++)
        {
            ...
            writer.Write(name);
            writer.Write(entry.Type == PackEntryType.Nop ? 0 : entry.CreationTime.ToFileTimeUtc());
            writer.Write(entry.Type == PackEntryType.Nop ? 0 : entry.ModifyTime.ToFileTimeUtc());
